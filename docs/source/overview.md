...
| dpi | int | DPI (dots per inch) for converting images from .pdfs. | 300 |
| max_pixels | int | Maximum number of pixels allowed for an image. If the image is larger than max_pixels, it will be resized. This option can be used to limit the size of the resulting presentation. | 1e8 |
